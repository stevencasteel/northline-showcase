...
const clamp = (value: number) => Math.min(100, Math.max(0, Number(value.toFixed(1))))

export function useComparisonSlider({ initialValue = 54, intentThreshold = 10, releaseDelayMs = 240 }: ComparisonSliderOptions = {}) {
  ...
  const onPointerMove = (event: PointerEvent<HTMLInputElement>) => {
    ...
    if (isDragging.current) updateFromClientX(event.currentTarget, event.clientX)
  }
