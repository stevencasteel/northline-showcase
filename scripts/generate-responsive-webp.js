...
const manifestPath = path.join(projectRoot, 'src/config/generatedResponsiveAssets.ts')
const widthTiers = [640, 960, 1440, 1920, 2560]
const sourceExtensions = /\.(png|jpe?g|webp)$/i
const force = process.argv.includes('--force')
const help = process.argv.includes('--help') || process.argv.includes('-h')
const targetArgs = process.argv.slice(2).filter((argument) => argument !== '--force' && argument !== '--help' && argument !== '-h')
...
const isSourceFile = (file) => sourceExtensions.test(file) && !path.basename(file).startsWith('._')
...
function encodingFor(sourcePath, metadata) {
  ...
  const isTexture = /texture|felt|steel|underlayment|cover-board|paper/.test(relative.toLowerCase())
  const isSharpGraphic = /badge|banner|copper-edge|sphere|plaque|map-frame/.test(baseName)
  ...
  if (metadata.hasAlpha) {
    return {
      ...
      alphaQuality: 100,
      effort: 6,
      treatment: isSharpGraphic ? 'alpha-full-quality-ui' : 'alpha-full-quality-photo',
    }
  }
...
function formatFor(sourcePath) {
  const relative = path.relative(assetsRoot, sourcePath).split(path.sep).join('/')
  const isAvifFamily = relative.startsWith('services/source_files/')
...
function outputPathFor(base, width, sourceWidth, format) {
  const outputBase = path.join(assetsRoot, base.replace('/assets/', '').split('/').join(path.sep))
...
function removeStaleVariants(base, keepPaths) {
  const outputDirectory = path.dirname(path.join(assetsRoot, base.replace('/assets/', '').split('/').join(path.sep)))
  const baseName = path.basename(base)
  if (!fs.existsSync(outputDirectory)) return
  const keep = new Set(keepPaths)
  for (const entry of fs.readdirSync(outputDirectory)) {
    if (!new RegExp(`^${baseName}(?:-(?:${widthTiers.join('|')}))?\\.(?:webp|avif)$`).test(entry)) continue
...
function dimensionsMatch(metadata, sourceMetadata, expected) {
  const ratioHeight = sourceMetadata.height * expected.width / sourceMetadata.width
  return metadata.width === expected.width && Math.abs(metadata.height - ratioHeight) <= 1
}
...
async function outputIsValid(outputPath, expected, sourceMetadata) {
  ...
  if (fs.statSync(outputPath).mtimeMs < fs.statSync(sourceMetadata.path).mtimeMs) return false
  try {
    const metadata = await sharp(outputPath).metadata()
    const expectedFormat = path.extname(outputPath).slice(1)
    const validFormats = expectedFormat === 'avif' ? ['avif', 'heif'] : [expectedFormat]
...
async function convertVariant(sourcePath, outputPath, targetWidth, sourceMetadata, encoding) {
  const expected = expectedDimensions(sourceMetadata.width, sourceMetadata.height, targetWidth)
  fs.mkdirSync(path.dirname(outputPath), { recursive: true })
  const image = sharp(sourcePath).resize({ width: targetWidth, fit: 'inside', withoutEnlargement: true })
  if (path.extname(outputPath) === '.avif') {
    await image.avif({ quality: 60, effort: encoding.effort }).toFile(outputPath)
  } else {
    await image.webp({
  ...
  const expectedFormat = path.extname(outputPath).slice(1)
  const validFormats = expectedFormat === 'avif' ? ['avif', 'heif'] : [expectedFormat]
  if (!validFormats.includes(metadata.format) || !dimensionsMatch(metadata, sourceMetadata, expected)) {
    throw new Error(`Invalid output dimensions for ${outputPath}: expected ${expected.width}x${expected.height}, got ${metadata.width}x${metadata.height}`)
  }
...
function writeManifest(families, mergeExisting = false) {
  ...
  Object.assign(entries, Object.fromEntries(families.map((family) => [family.base, {
  ...
  const contents = `// Generated by scripts/generate-responsive-webp.js. Do not edit by hand.\nexport const RESPONSIVE_ASSETS = ${JSON.stringify(entries, null, 2)} as const\n`
  fs.writeFileSync(manifestPath, contents)
}

function resolveTarget(target) {
  if (target.startsWith('/assets/')) return path.join(projectRoot, 'public', target.slice(1))
  return path.resolve(projectRoot, target)
}

function printHelp() {
  console.log(`Usage: npm run images:webp [-- --force] [source-file ...]
...
Examples:
  npm run images:webp
  npm run images:webp -- public/assets/navbar/source_files/nav_email_default.png
  npm run images:webp -- /assets/navbar/source_files/nav_email_hover.png --force`)
}
...
async function main() {
  ...
  const allSourceFiles = walk(assetsRoot)
    ...
    .sort()
  const sourceFiles = targetArgs.length === 0
    ? allSourceFiles
    : targetArgs.map(resolveTarget).map((file) => {
      if (!allSourceFiles.includes(file)) {
        throw new Error(`Target is not a supported raster source master: ${file}`)
      }
      return file
    })
  ...
  for (const sourcePath of sourceFiles) {
    ...
    const variantReport = []
    const outputPaths = targets.map((targetWidth) => outputPathFor(family.base, targetWidth, sourceMetadata.width, format))
    ...
    const variants = await mapConcurrent(targets, async (targetWidth) => {
      const outputPath = outputPathFor(family.base, targetWidth, sourceMetadata.width, format)
      const expected = expectedDimensions(sourceMetadata.width, sourceMetadata.height, targetWidth)
      if (!force && await outputIsValid(outputPath, expected, sourceMetadata)) {
        return { path: outputPath, status: 'skipped', ...expected, bytes: fs.statSync(outputPath).size }
      }
      const result = await convertVariant(sourcePath, outputPath, targetWidth, sourceMetadata, encoding)
    ...
    const variantSummary = variantReport.map((variant) => `${variant.width}x${variant.height}/${Math.round(variant.bytes / 1024)}KiB`).join(', ')
    console.log(`${family.base} <- ${path.relative(projectRoot, sourcePath).split(path.sep).join('/')} [${sourceMetadata.width}x${sourceMetadata.height}${sourceMetadata.hasAlpha ? ', alpha' : ''}] ${variantSummary}`)
  }

  writeManifest(families, targetArgs.length > 0)
  const generatedFormats = families.reduce((counts, family) => {
    counts[family.outputFormat] = (counts[family.outputFormat] || 0) + family.variants.filter((variant) => variant.status === 'generated').length
    return counts
  }, {})
  ...
  console.log(`Source bytes: ${totalBefore}; deployed responsive image bytes: ${totalAfter}`)
